...
public class PrincipalOauth2UserService extends DefaultOAuth2UserService{
	...
	*/
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		 ...
		 * EndPoint 결과 즉 OAuth2User 정보를 가진 객체를 리턴
		*/
		OAuth2User oAuth2User = super.loadUser(userRequest);
		
		/*
		 * Provider 정보(클라이언트 ID, 클라이언트 시크릿, 클라이언트 name 등을 가지고 있다
		*/
	...
	private User getOAuth2User(String provider, Map<String, Object> attributes) throws OAuth2AuthenticationException {
		...
		if(user == null) {	// null이면 회원가입이 필요하다
			user = User.builder()
